...
def verify_commit_pages(pages: Any, *, expected_count: int, expected_head: str) -> int:
    ...
    if not 1 <= expected_count <= _MAX_COMMITS:
    ...
    for commit in commits:
        ...
        if (name, email) not in _trailer_signoffs(_message(details.get("message"))):
...
def verify_commit_file(path: str | Path, *, expected_count: int, expected_head: str) -> int:
    ...
    try:
        ...
        pages = json.loads(source.read_bytes(), object_pairs_hook=_pairs, parse_constant=_constant)
    except DCOError:
        raise
    except (OSError, UnicodeError, json.JSONDecodeError, RecursionError) as error:
        raise DCOError(f"cannot read pull-request commit metadata: {error}") from error
    return verify_commit_pages(pages, expected_count=expected_count, expected_head=expected_head)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if len(arguments) != 3:
        print(
            "usage: python dco.py PULL_COMMITS.json EXPECTED_COUNT EXPECTED_HEAD", file=sys.stderr
        )
        return 2
    try:
        ...
        count = verify_commit_file(
            arguments[0], expected_count=int(arguments[1]), expected_head=arguments[2]
        )
