"""Fail-closed wheel and source-archive checks for release evidence."""
...
class DistributionArchiveError(ValueError):
    """A distribution archive is ambiguous, unsafe, or outside release limits."""
...
def audit_sdist(archive: tarfile.TarFile) -> frozenset[str]:
    """Validate an sdist and return regular-file names below its single root."""
    ...
    for count, member in enumerate(archive, start=1):
        ...
        _record(path, directory=directory, observed=observed, files=files, parents=parents)
