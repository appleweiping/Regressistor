...
from regressistor.distribution_audit import DistributionArchiveError, audit_sdist, audit_wheel
...
def test_distribution_auditors_accept_canonical_regular_files() -> None:
    with _tar(
        [
            ("regressistor-1/", b"", tarfile.DIRTYPE),
            ("regressistor-1/src/", b"", tarfile.DIRTYPE),
            ("regressistor-1/src/module.py", b"value = 1\n", None),
        ]
    ) as archive:
        assert audit_sdist(archive) == {"src/module.py"}
    with _zip(
        [
            ("regressistor/", b"", stat.S_IFDIR | 0o755),
            ("regressistor/module.py", b"value = 1\n", stat.S_IFREG | 0o644),
        ]
    ) as archive:
        assert audit_wheel(archive) == {"regressistor/module.py"}
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ([("root/file", b"x", None), ("root/file/child", b"x", None)], "prefix collision"),
...
def test_wheel_rejects_encrypted_entries() -> None:
