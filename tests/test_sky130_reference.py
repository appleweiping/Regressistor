...
FIXTURE = ROOT / "benchmarks" / "fixtures" / "simcairn-sky130-ngspice42.json"
POLICY = ROOT / "benchmarks" / "sky130-policy.toml"
METRIC_UNITS = {
    "gain_100khz": "1",
    "output_pp_v": "V",
    "output_v": "V",
...
def test_sky130_reference_has_all_identities_metrics_units_and_passes_self_gate() -> None:
    bundle = load_bundle(FIXTURE)
    expected = {
        (corner, vdd, temperature)
        for corner, vdd, temperature in itertools.product(
            ("tt", "ss", "ff"), ("1.62", "1.80", "1.98"), ("-40", "27", "125")
        )
    }
    observed = {
...
def test_sky130_policy_rejects_numeric_regression_and_missing_case() -> None:
